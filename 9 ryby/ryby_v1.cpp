...
int main()
{
	...
	//Vyzkoušet všechny možnosti x a y odkud by mohla loď plout a zjistit možnost s nejvíce kolizemi
	unsigned int maxcollisions = 0;
	for (int xxx = 0; xxx < maxx; xxx++) {
		for (int yyy = 0; yyy < maxy; yyy++) {
			//Úsečka s od bodu xxx,yyy podle směru dir
			Line s = {{xxx,yyy},{dir.x*max(maxx,maxy)+xxx,dir.y*max(maxx,maxy)+yyy}};
